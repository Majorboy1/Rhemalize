...
class _UploadSeriesScreenState extends State<UploadSeriesScreen> {
  ...
  Future<void> _handlePublish() async {
    ...
    // Validation: Check if audio is selected for all episodes based on platform
    bool missingAudio = _episodeDrafts.any((e) {
    ...
    try {
      ...
      if (_selectedMessageType == MessageType.series) {
        ...
        for (int i = 0; i < _episodeDrafts.length; i++) {
          var e = _episodeDrafts[i];
          episodesData.add({
            'title': e.titleController.text.trim(),
            'audioFile': e.audioFile, // Used by Mobile logic in Provider
            'audioBytes': e.webAudioBytes, // Used by Web logic in Provider
  ...
  Future<void> _pickAudioForEpisode(int index) async {
    FilePickerResult? result = await FilePicker.platform.pickFiles(
      type: FileType.audio,
      allowMultiple: false,
      withData: true, // Essential for Web (populates bytes)
    );
    ...
    if (result != null) {
      setState(() {
        ...
        // Handle Mobile/Desktop
          ...
          // On some mobile devices, bytes might also be available
          _episodeDrafts[index].webAudioBytes = platformFile.bytes;
        }
      });
    }
  ...
  Widget _buildEpisodeCard(int index, Color bgColor, bool isDark) {
    return Card(
      key: ValueKey(_episodeDrafts[index].id),
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: _buildTextField(
                      _episodeDrafts[index].titleController,
                      "Episode Title",
            ...
            InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: _episodeDrafts[index].fileName != null
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        _episodeDrafts[index].fileName ?? "Tap to Select Audio",
                        style: TextStyle(
                          color: _episodeDrafts[index].fileName != null
                              ? Colors.green
                              : Colors.grey,
                    ...
                    if (_episodeDrafts[index].fileName != null)
